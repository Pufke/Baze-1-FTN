...
FindRecordResult findRecord(FILE *pFile, int key) {
    ...
    int q;                                                          // indeks trenutnog sloga u baketu
...
int insertRecord(FILE *pFile, Record record) {
    ...
    if (alreadyExistsForInsert(findResult)) {                           // ukoliko slog sa datim kljucem ne postoji
        return -1;
    }
...
int removeRecordPhysically(FILE *pFile, int key) {
    ...
    while (move) {
        ...
        if (bucket.records[q].status == EMPTY) {
            ...
            while (!found && move) {
                if (q == BUCKET_SIZE - 1) {
                    bucketIndex = (bucketIndex + 1) % B;                    // ukoliko se doslo do kraja baketa
