...
public class PozoristeDAOImpl implements PozoristeDAO {
	...
	public void saveAll(Iterable<Pozoriste> entities) throws SQLException {
		String insertCommand = "insert into pozoriste (nazivpoz, adresapoz, sajt, mesto_idm,idpoz) values (?,?,?,?,?)";
		String updateCommand = "update pozooriste set nazivpoz=?, adresapoz=?, sajt=?, mesto_idm=? where idpoz=?";
		...
		try(Connection connection = ConnectionUtil_HikariCP.getConnection();
			...
			connection.setAutoCommit(false); //iskljucujmeo autoCommit iz razloga sto imamo vise entiteta 
			//pa zelimo da prvo dodamo sve entitete pa tek onda comitujemo
